...
const DesignInsights = () => {
  ...
  const articles = blogPosts.slice(0, 3).map(post => ({
    ...
    author: {
      ...
      avatar: post.author?.image || '/images/Desgin-Insights/10.webp'
    },
    date: post.date,
    slug: post.slug
  }));
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {articles.map((article, idx) => (
            <Link href={`/blog/${article.slug}`} key={idx} className="block">
              <div className="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col hover:shadow-xl transition-shadow duration-300 cursor-pointer">
                ...
                <div className="p-6 flex flex-col flex-1">
                  <h3 className={`text-xl font-bold mb-2 ${article.tag === 'TIPS' ? 'text-red-600' : 'text-gray-900'}`}>
                    {article.title}
                  </h3>
