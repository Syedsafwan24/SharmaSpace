...
const HeroCard = ({ project }) => (
  <article className="relative group h-[400px] rounded-2xl overflow-hidden shadow-lg">
    ...
    <div className="relative z-10 flex flex-col justify-end h-full p-6 text-white">
      ...
      {project.description && (
        <p className="mt-2 text-sm leading-relaxed line-clamp-3">{project.shortDescription || project.description}</p>
      )}
      <Link
...
const PortfolioSection = () => {
  ...
  const featuredProjects = portfolioProjects.filter(p => p.featured).slice(0, 5);
  const projects = featuredProjects.length >= 5 ? featuredProjects : portfolioProjects.slice(0, 5);
