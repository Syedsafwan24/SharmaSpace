...
const EditTestimonialForm = ({ testimonial, onClose }) => {
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold text-[#1C1C1C] mb-4">{isEditMode ? 'Edit Testimonial' : 'Add New Testimonial'}</h2>
      <p className="text-gray-600 mb-6">{isEditMode ? 'Update the details for this testimonial.' : 'Fill in the details for the new testimonial.'}</p>

      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          <label htmlFor="name" className="block text-gray-700 text-sm font-bold mb-2">Client Name</label>
          <input
            type="text"
            id="name"
            name="name"
        ...
        <div className="mb-4">
          <label htmlFor="image" className="block text-gray-700 text-sm font-bold mb-2">Client Image URL (Optional)</label>
          <input
            type="text"
            id="image"
            name="image"
          ...
          {formData.image && (
            <div className="mt-2 w-20 h-20 rounded-full overflow-hidden border border-gray-200">
              <Image src={formData.image} alt="Client Preview" width={80} height={80} className="object-cover w-full h-full" />
            </div>
          )}
        </div>

        <div className="mb-4">
          <label htmlFor="text" className="block text-gray-700 text-sm font-bold mb-2">Testimonial Text</label>
          <textarea
            id="text"
            name="text"
            value={formData.text}
